...
def plt_beta(perc_data):
    ...
    plt.savefig("Line Beta P(p) vs L (p--%8.7f) (Range-- %d-%d).png" %(p, g1, g2), dpi=400)
    plt.show()
    plt.close()
...
def plt_gamma(post_transcript):
    ...
    g= sea.lineplot(data=zerodark30, x="L" , y=r"$\langle S[p] \rangle$", estimator='mean', ci='sd', marker="s", err_style="bars")
    ...
    plt.savefig("Bar Gamma S(p) vs L (p--%8.7f) (Range-- %d-%d).png" %(post_transcript[0,0], g1, g2), dpi=400)
    plt.show()
    plt.close()
